 ...
 *     @brief Set of hotels whose rating averages need to be calculated.
...
int __q03_generate_statistics_foreach_reservation(void                *user_data,
    ...
    if (!set_belongs(foreach_data->hotels_to_average, GUINT_TO_POINTER(hotel_id)))
...
void *__q03_generate_statistics(database_t *database, query_instance_t *instances, size_t n) {
    ...
    /*
     * Generate a set of all hotel IDs that will be asked for in queries, to avoid storing
     * rating information about all hotels.
     */
    set_t *hotels_to_average = set_create(g_direct_equal, g_direct_hash);
    for (size_t i = 0; i < n; ++i) {
        hotel_id_t *hotel_id = (hotel_id_t *) query_instance_get_argument_data(instances);
        set_add(hotels_to_average, GUINT_TO_POINTER(*hotel_id));
